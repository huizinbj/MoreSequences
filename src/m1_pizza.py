...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(110.0, 20.0),  # All numbers are approximate.
                rg.Point(95, 28.7),
                rg.Point(95, 11.3), ]
    circle = rg.Circle(rg.Point(100, 20), 10)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(points)):
        plottedcircle = rg.Circle(points[k], 10)
        plottedcircle.fill_color = color
        plottedcircle.attach_to(window)
...
def run_test_pizza():
    ...
    title = 'PIZZA test 4:  2 slices, thick (thickness=10) blue lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 10
    pizza(window, circle, 2, 'blue', 10)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(100, 100), 80)
    circle.outline_thickness = 10
    polygon(window, circle, 4, 'blue', 10)
    window.continue_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    circle.fill_color = 'blue'
    fancy_polygon(window, circle, 40, 20, 'lime green', 6)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    circle.attach_to(window)
    for k in range(len(points)):
        hopk = (k + hops_to_next_point) % number_of_lines
        start = points[k]
        end = points[hopk]
